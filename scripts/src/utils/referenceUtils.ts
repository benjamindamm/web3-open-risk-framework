...
export function extractReferences(content: string): ExtractedReferences {
  ...
  const indicatorMatches = content.match(/I:[A-Z0-9_]+(?:\.FIDLEG)?/g);
  if (indicatorMatches) {
    indicatorRefs.push(...new Set(indicatorMatches));
  }

  // Find M: references (including .FIDLEG extensions)
  const measureMatches = content.match(/M:[A-Z0-9_]+(?:\.FIDLEG)?/g);
  if (measureMatches) {
  ...
  const assessmentMatches = content.match(/\bA:[A-Z0-9_]+(?:\.FIDLEG)?\b/g);
  ...
  const riskMatches = content.match(/R:[A-Z0-9_]+(?:\.FIDLEG)?/g);
  if (riskMatches) {
...
export function validateAllReferences(
  catalogueDir: string,
  baseDir: string,
  title: string = 'Validating References in Risk Framework'
): { filesChecked: number; filesWithErrors: number; totalErrors: number; allErrors: FileErrors[] } {
  ...
  for (const filePath of catalogueFiles) {
    ...
    const errors = validateFileReferences(
      filePath, 
      availableIndicators, 
      availableMeasures, 
      availableAssessments, 
